...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = __webpack_require__(/*! ./src/content-scripts/main.js */1);
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.prefs = exports.currentProvider = exports.host = undefined;
	...
	var _clientAPI = __webpack_require__(/*! ./clientAPI */ 5);

	var clientAPI = _interopRequireWildcard(_clientAPI);

	var _providerSpecific = __webpack_require__(/*! ./providerSpecific */ 13);

	var providers = _interopRequireWildcard(_providerSpecific);

	var _decryptFrame = __webpack_require__(/*! ./decryptFrame */ 14);

	var _decryptFrame2 = _interopRequireDefault(_decryptFrame);

	var _verifyFrame = __webpack_require__(/*! ./verifyFrame */ 16);

	var _verifyFrame2 = _interopRequireDefault(_verifyFrame);

	var _importFrame = __webpack_require__(/*! ./importFrame */ 17);

	var _importFrame2 = _interopRequireDefault(_importFrame);

	var _encryptFrame = __webpack_require__(/*! ./encryptFrame */ 18);

	var _encryptFrame2 = _interopRequireDefault(_encryptFrame);
	...
	const NAME = 'mainCS-' + _mvelo2.default.util.getHash();
	...
	let host = exports.host = null;
	let currentProvider = exports.currentProvider = null;
	let prefs = exports.prefs = null;
	...
	(0, _jquery2.default)(document).ready(connect);

	function init(preferences, watchlist) {
	  exports.prefs = prefs = preferences;
	  watchList = watchlist;
	  detectHost();
	  ...
	  providers.init();
	  exports.currentProvider = currentProvider = providers.get(host);
	...
	function findPGPTag() {
	  ...
	  nodeList = (0, _jquery2.default)(nodeList).filter(function () {
	    var element = (0, _jquery2.default)(this);
	    // visibility check does not work on text nodes
	    return element.parent().is(':visible') &&
	    // no elements within editable elements
	    element.parents('[contenteditable], textarea').length === 0 && this.ownerDocument.designMode !== 'on';
	  });
	...
	function findEditable() {
	  ...
	  var editable = (0, _jquery2.default)('[contenteditable], textarea').filter(':visible').not('body');
	  var iframes = (0, _jquery2.default)('iframe').filter(':visible');
	  ...
	  var dynFrames = iframes.filter(function () {
	    ...
	    return src === undefined || src === '' || /^javascript.*/.test(src) || /^about.*/.test(src);
	  });
	  // find editable elements inside dynamic iframe (content script is not injected here)
	  dynFrames.each(function () {
	    var content = (0, _jquery2.default)(this).contents();
	    ...
	    .data(_mvelo2.default.DYN_IFRAME, true)
	    // add iframe element
	    .data(_mvelo2.default.IFRAME_OBJ, (0, _jquery2.default)(this));
	    ...
	    if (content.attr('designMode') === 'on' || content.find('body[contenteditable]').length !== 0) {
	      ...
	      editable = editable.add((0, _jquery2.default)(this));
	    } else {
	  ...
	  var anchor = (0, _jquery2.default)('<a/>');
	  var editableBody = iframes.not(dynFrames).filter(function () {
	    var frame = (0, _jquery2.default)(this);
	    ...
	    if (anchor.attr('href', frame.attr('src')).prop('hostname') === document.location.hostname) {
	      try {
	        ...
	        if (content.attr('designMode') === 'on' || content.find('body[contenteditable]').length !== 0) {
	          ...
	          //content.find('body').off("contextmenu").on("contextmenu", onContextMenu);
	          // mark body as 'inside iframe'
	          content.find('body').data(_mvelo2.default.IFRAME_OBJ, frame);
	          return true;
	        } else {
	          return false;
	        }
	  ...
	  editable = editable.filter(function () {
	    return (0, _jquery2.default)(this).height() > MIN_EDIT_HEIGHT;
	  });
	  return editable;
	}
	...
	function getMessageType(armored) {
	  if (/END\sPGP\sMESSAGE/.test(armored)) {
	    return _mvelo2.default.PGP_MESSAGE;
	  } else if (/END\sPGP\sSIGNATURE/.test(armored)) {
	    return _mvelo2.default.PGP_SIGNATURE;
	  } else if (/END\sPGP\sPUBLIC\sKEY\sBLOCK/.test(armored)) {
	    return _mvelo2.default.PGP_PUBLIC_KEY;
	  } else if (/END\sPGP\sPRIVATE\sKEY\sBLOCK/.test(armored)) {
	    return _mvelo2.default.PGP_PRIVATE_KEY;
	  }
	}
	...
	function attachExtractFrame(element) {
	  ...
	  var newObj = element.filter(function () {
	    return !isAttached((0, _jquery2.default)(this).parent());
	  });
	  // create new decrypt frames for new discovered PGP tags
	  newObj.each(function (index, element) {
	    try {
	      // parent element of text node
	      var pgpEnd = (0, _jquery2.default)(element).parent();
	      switch (getMessageType(pgpEnd.text())) {
	        case _mvelo2.default.PGP_MESSAGE:
	          var dFrame = new _decryptFrame2.default(prefs);
	          dFrame.attachTo(pgpEnd);
	          break;
	        case _mvelo2.default.PGP_SIGNATURE:
	          var vFrame = new _verifyFrame2.default(prefs);
	          vFrame.attachTo(pgpEnd);
	          break;
	        case _mvelo2.default.PGP_PUBLIC_KEY:
	          var imFrame = new _importFrame2.default(prefs);
	...
	function attachEncryptFrame(element, expanded) {
	  ...
	  var newObj = element.filter(function () {
	    if (expanded) {
	      ...
	      if (element.data(_mvelo2.default.FRAME_STATUS) === _mvelo2.default.FRAME_ATTACHED) {
	        // trigger expand state of attached frames
	        element.data(_mvelo2.default.FRAME_OBJ).showEncryptDialog();
	        return false;
	      } else {
	        return true;
	      }
	      ...
	      return !isAttached((0, _jquery2.default)(this));
	    }
	  });
	  // create new encrypt frames for new discovered editable fields
	  newObj.each(function (index, element) {
	    var eFrame = new _encryptFrame2.default(prefs);
	    eFrame.attachTo((0, _jquery2.default)(element), { expanded: expanded });
	  });
	}
	...
	function isAttached(element) {
	  var status = element.data(_mvelo2.default.FRAME_STATUS);
	  switch (status) {
	    case _mvelo2.default.FRAME_ATTACHED:
	    case _mvelo2.default.FRAME_DETACHED:
	      return true;
	    default:
	      return false;
	  }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _main = __webpack_require__(/*! ./main */ 1);

	var _decryptContainer = __webpack_require__(/*! ./decryptContainer */ 6);

	var _decryptContainer2 = _interopRequireDefault(_decryptContainer);

	var _editorContainer = __webpack_require__(/*! ./editorContainer */ 7);

	var _editorContainer2 = _interopRequireDefault(_editorContainer);

	var _optionsContainer = __webpack_require__(/*! ./optionsContainer */ 8);

	var _optionsContainer2 = _interopRequireDefault(_optionsContainer);

	var _keyGenContainer = __webpack_require__(/*! ./keyGenContainer */ 9);

	var _keyGenContainer2 = _interopRequireDefault(_keyGenContainer);

	var _keyBackupContainer = __webpack_require__(/*! ./keyBackupContainer */ 10);

	var _keyBackupContainer2 = _interopRequireDefault(_keyBackupContainer);

	var _restoreBackupContainer = __webpack_require__(/*! ./restoreBackupContainer */ 11);

	var _restoreBackupContainer2 = _interopRequireDefault(_restoreBackupContainer);

	var _syncHandler = __webpack_require__(/*! ./syncHandler */ 12);
	...
	let active = exports.active = false;
	// must be a singelton
	...
	function init() {
	  ...
	  if (apiTag) {
	    if (apiTag.dataset.version !== _main.prefs.version) {
	      window.setTimeout(function () {
	        window.dispatchEvent(new CustomEvent('mailvelope-disconnect', { detail: { version: _main.prefs.version } }));
	      }, 1);
	    }
	    return;
	  }
	  ...
	  if (!window.mailvelope) {
	    (0, _jquery2.default)('<script/>', {
	      id: 'mailvelope-api',
	      src: _mvelo2.default.extension.getURL('client-API/mailvelope-client-api.js'),
	      'data-version': _main.prefs.version
	    }).appendTo((0, _jquery2.default)('head'));
	  }
	}
	...
	function eventListener(event) {
	  if (event.origin !== window.location.origin || event.data.mvelo_extension || !event.data.mvelo_client) {
	    return;
	  }
	  ...
	  try {
	    ...
	    if (data && data.identifier) {
	      if (data.identifier.indexOf(_mvelo2.default.KEYRING_DELIMITER) !== -1) {
	        throw { message: 'Identifier invalid.', code: 'INVALID_IDENTIFIER' };
	      }
	      keyringId = _main.host + _mvelo2.default.KEYRING_DELIMITER + data.identifier;
	    }
	    switch (event.data.event) {
	      case 'get-version':
	        reply(event.data.id, null, _main.prefs.version);
	        break;
	      case 'get-keyring':
	...
	function displayContainer(selector, armored, keyringId, options, callback) {
	  var container, error;
	  switch ((0, _main.getMessageType)(armored)) {
	    case _mvelo2.default.PGP_MESSAGE:
	      container = new _decryptContainer2.default(selector, keyringId, options);
	      break;
	    case _mvelo2.default.PGP_SIGNATURE:
	...
	function editorContainer(selector, keyringId) {
	  ...
	  var container = new _editorContainer2.default(selector, keyringId, options);
	  containers.set(container.id, container);
	  container.create(callback);
	}

	function settingsContainer(selector, keyringId, options, callback) {
	  var container = new _optionsContainer2.default(selector, keyringId, options);
	  containers.set(container.id, container);
	  container.create(callback);
	}
	...
	function keyGenContainer(selector, keyringId) {
	  let options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
	  let callback = arguments[3];

	  options.keySize = options.keySize || 2048;
	  var container = new _keyGenContainer2.default(selector, keyringId, options);
	  containers.set(container.id, container);
	  container.create(callback);
	}

	function keyBackupContainer(selector, keyringId) {
	  let options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
	  let callback = arguments[3];

	  var container = new _keyBackupContainer2.default(selector, keyringId, options);
	  containers.set(container.id, container);
	  container.create(callback);
	}

	function restoreBackupContainer(selector, keyringId) {
	  let options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
	  let callback = arguments[3];

	  var container = new _restoreBackupContainer2.default(selector, keyringId, options);
	  containers.set(container.id, container);
	  container.create(callback);
	}
	...
	function importPublicKey(keyringId, armored, callback) {
	  ...
	  switch ((0, _main.getMessageType)(armored)) {
	    case _mvelo2.default.PGP_PUBLIC_KEY:
	      // ok
	      break;
	    case _mvelo2.default.PGP_PRIVATE_KEY:
	...
	function addSyncHandler(keyringId, callback) {
	  syncHandler = syncHandler || new _syncHandler2.default(keyringId);
...
/***/ function(module, exports, __webpack_require__) {
	...
	class DecryptContainer {
	  constructor(selector, keyringId, options) {
	    ...
	    this.id = _mvelo2.default.util.getHash();
	    this.name = 'decryptCont-' + this.id;
	    this.port = _mvelo2.default.extension.connect({ name: this.name });
	  ...
	  create(armored, done) {
	    ...
	    if (_mvelo2.default.crx) {
	      url = _mvelo2.default.extension.getURL('components/decrypt-inline/decryptInline.html?id=' + this.id);
	...
	exports.default = DecryptContainer;
...
/***/ function(module, exports, __webpack_require__) {
	...
	class EditorContainer {
	  ...
	  create(done) {
	    ...
	    if (_mvelo2.default.crx) {
	      url = _mvelo2.default.extension.getURL('components/editor/editor.html?id=' + this.id + quota + '&embedded=true');
	    } else if (_mvelo2.default.ffa) {
	  ...
	  processOptions() {
	    var error;
	    if (this.options.quotedMail && (0, _main.getMessageType)(this.options.quotedMail) !== _mvelo2.default.PGP_MESSAGE || this.options.armoredDraft && (0, _main.getMessageType)(this.options.armoredDraft) !== _mvelo2.default.PGP_MESSAGE) {
	      error = new Error('quotedMail or armoredDraft parameter need to be a PGP message.');
	      error.code = 'WRONG_ARMOR_TYPE';
	      return error;
	    }
	    if (this.options.armoredDraft && (this.options.predefinedText || this.options.quotedMail || this.options.quotedMailIndent || this.options.quotedMailHeader)) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	class OptionsContainer {
	  ...
	  create(done) {
	    ...
	    if (_mvelo2.default.crx) {
	      url = _mvelo2.default.extension.getURL('app/app.html?' + options + '#keyring');
	    } else if (_mvelo2.default.ffa) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	class KeyGenContainer {
	  ...
	  create(done) {
	    ...
	    if (_mvelo2.default.crx) {
	      url = _mvelo2.default.extension.getURL('components/generate-key/keyGenDialog.html?id=' + this.id);
	    } else if (_mvelo2.default.ffa) {
	...
	exports.default = KeyGenContainer;
...
/***/ function(module, exports, __webpack_require__) {
	...
	class KeyBackupContainer {
	  ...
	  create(done) {
	    ...
	    if (_mvelo2.default.crx) {
	      url = _mvelo2.default.extension.getURL('components/key-backup/keyBackupDialog.html?id=' + this.id);
	    } else if (_mvelo2.default.ffa) {
	...
	exports.default = KeyBackupContainer;
...
/***/ function(module, exports, __webpack_require__) {
	...
	class RestoreBackupContainer {
	  ...
	  create(done) {
	    ...
	    if (_mvelo2.default.crx) {
	      url = _mvelo2.default.extension.getURL('components/restore-backup/restoreBackupDialog.html?id=' + this.id);
	    } else if (_mvelo2.default.ffa) {
	...
	exports.default = RestoreBackupContainer;
	...
	class SyncHandler {
	  ...
	  registerEventListener() {
	    this.port.onMessage.addListener(function (msg) {
	      switch (msg.event) {
	        case 'sync-event':
	          (0, _clientAPI.postMessage)('sync-event', null, msg, null);
	          break;
	        default:
	          console.log('unknown event', msg);
	      }
...
/***/ function(module, exports, __webpack_require__) {
	...
	class Gmail {
	   ...
	   */
	  getRecipients() {
	    return Promise.resolve(dom.getAttr((0, _jquery2.default)('.oL.aDm span[email], .vR span[email]'), 'email'));
	  }
	  ...
	  setRecipients(_ref) {
	    ...
	    let recipients = _ref$recipients === undefined ? [] : _ref$recipients;

	    // find the relevant elements in the Gmail interface
	    var displayArea = (0, _jquery2.default)('.aoD.hl'); // email display only area
	    var tagRemove = (0, _jquery2.default)('.fX .vR .vM'); // email tags remove button
	    var input = (0, _jquery2.default)('.fX .vO'); // the actual recipient email address text input (a textarea)
	    var subject = (0, _jquery2.default)('.aoT'); // subject field
	    var editor = (0, _jquery2.default)('.aO7 .Am'); // editor
	    input.val('');
	    dom.setFocus(displayArea).then(function () {
	      tagRemove.click();
	...
	class Yahoo {
	   ...
	   */
	  getRecipients() {
	    return Promise.resolve(dom.getAttr((0, _jquery2.default)('.compose-header span[data-address]'), 'data-address'));
	  }
	   ...
	   */
	  setRecipients(_ref2) {
	    var _ref2$recipients = _ref2.recipients;
	    let recipients = _ref2$recipients === undefined ? [] : _ref2$recipients;

	    // remove existing recipients
	    (0, _jquery2.default)('.compose-header li.hLozenge').remove();
	    // enter address text into input
	    var text = joinEmail(recipients);
	    var input = (0, _jquery2.default)('.compose-header #to .recipient-input input');
	    input.val(text);
	    // trigger change event by switching focus
	    dom.setFocus(input).then(function () {
	      // set focus to subject field, or to compose area in the reply case
	      dom.setFocus((0, _jquery2.default)('#subject-field').is(':visible') ? (0, _jquery2.default)('#subject-field') : (0, _jquery2.default)('.compose-message .cm-rtetext'));
	    });
	  }
	...
	class Outlook {
	  getRecipients(editElement) {
	    ...
	    return _mvelo2.default.util.sequential(this.extractPersona.bind(this), personas);
	  }
	      ...
	      const observer = new MutationObserver(mutations => {
	        mutations.forEach(mutation => {
	          ...
	          const searchInterval = setInterval(() => {
	            ...
	            if (personaCard.length && (0, _jquery2.default)(personaCard).text().match(HAS_EMAIL)) {
	  ...
	  extractPersona(pane) {
	    ...
	    return this.waitForPersonaCard(() => (0, _jquery2.default)(pane).click()).then((_ref3) => {
	      let personaCard = _ref3.personaCard,
	          addedNode = _ref3.addedNode;

	      // hide persona popup
	      (0, _jquery2.default)(addedNode).hide();
	      return dom.getText((0, _jquery2.default)(personaCard).find('span'));
	    }).catch(() => []);
	  }

	  setRecipients(_ref4) {
	    var _ref4$recipients = _ref4.recipients;
	    let recipients = _ref4$recipients === undefined ? [] : _ref4$recipients,
	        editElement = _ref4.editElement;
	    ...
	    dom.setFocus(input).then(() => input.val(text));
	  }
	...
	function parseEmail(elements, extract) {
	  ...
	  elements.each(function () {
	    const value = extract((0, _jquery2.default)(this));
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _extractFrame = __webpack_require__(/*! ./extractFrame */ 15);

	var _extractFrame2 = _interopRequireDefault(_extractFrame);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	class DecryptFrame extends _extractFrame2.default {
	  constructor(prefs) {
	    super(prefs);
	  ...
	  _clickHandler() {
	    super._clickHandler();
	    if (this._displayMode == _mvelo2.default.DISPLAY_INLINE) {
	      this._inlineDialog();
	    } else if (this._displayMode == _mvelo2.default.DISPLAY_POPUP) {
	      this._popupDialog();
	    }
	    return false;
	  }
	  ...
	  _inlineDialog() {
	    this._dDialog = (0, _jquery2.default)('<iframe/>', {
	      id: 'dDialog-' + this.id,
	    ...
	    this._port.onMessage.addListener(msg => {
	      ...
	      switch (msg.event) {
	        ...
	        case 'dialog-cancel':
	          ...
	          break;
	        case 'get-armored':
	          this._getEmailSender().then(sender => {
	            sender = sender.map(person => person.email);
	            sender = _mvelo2.default.util.deDup(sender);
	            this._port.postMessage({
	              event: 'set-armored',
...
/***/ function(module, exports, __webpack_require__) {
	...
	class ExtractFrame {
	  constructor(prefs) {
	    ...
	    this._currentProvider = _main.currentProvider;
	  }
	  ...
	  _init(pgpEnd) {
	    ...
	    for (var i = 0; i < maxNesting; i++) {
	      if (this._pgpStartRegex.test(this._pgpElement.text()) && this._pgpElement.width() > 0) {
	        beginFound = true;
	        break;
	      }
	    ...
	    this._pgpEnd.data(_mvelo2.default.FRAME_STATUS, _mvelo2.default.FRAME_ATTACHED);
	    // store frame obj in pgpText tag
	    this._pgpEnd.data(_mvelo2.default.FRAME_OBJ, this);
	  ...
	  _renderFrame() {
	    this._eFrame = (0, _jquery2.default)('<div/>', {
	      id: 'eFrame-' + this.id,
	    ...
	    if (this._pgpElement.height() > _mvelo2.default.LARGE_FRAME) {
	    ...
	    (0, _jquery2.default)(window).resize(this._setFrameDim.bind(this));
	  ...
	  _closeFrame(finalClose) {
	    this._eFrame.fadeOut(function () {
	      ...
	      if (finalClose === true) {
	        ...
	        this._pgpEnd.data(_mvelo2.default.FRAME_STATUS, null);
	      } else {
	        this._pgpEnd.data(_mvelo2.default.FRAME_STATUS, _mvelo2.default.FRAME_DETACHED);
	      }
	      this._pgpEnd.data(_mvelo2.default.FRAME_OBJ, null);
	    }.bind(this));
	    return false;
	  }
	  ...
	  _establishConnection() {
	    this._port = _mvelo2.default.extension.connect({ name: this._ctrlName });
	  ...
	  _getPGPMessage() {
	    ...
	    msg = _mvelo2.default.util.decodeQuotedPrint(msg);
	    return msg;
	  }
...
/***/ function(module, exports, __webpack_require__) {
	...
	class VerifyFrame extends _extractFrame2.default {
	  constructor(prefs) {
	    super(prefs);
	    this._displayMode = prefs.security.display_decrypted;
	    this._vDialog = null;
	  ...
	  _inlineDialog() {
	    this._vDialog = (0, _jquery2.default)('<iframe/>', {
	      id: 'vDialog-' + this.id,
	      'class': 'm-frame-dialog',
	      frameBorder: 0,
	      scrolling: 'no'
	    ...
	    var url;
	    if (_mvelo2.default.crx) {
	      url = _mvelo2.default.extension.getURL('components/verify-inline/verifyInline.html?id=' + this.id);
	    } else if (_mvelo2.default.ffa) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	class ImportFrame extends _extractFrame2.default {
	  constructor(prefs) {
	    super(prefs);
...
/***/ function(module, exports, __webpack_require__) {
	...
	class EncryptFrame extends _mvelo2.default.EventHandler {
	  constructor() {
	    super();
	    this.id = _mvelo2.default.util.getHash();
	    ...
	    this._editorType = _mvelo2.default.PLAIN_TEXT; //prefs.general.editor_type;
	  ...
	  attachTo(element, options) {
	    _jquery2.default.extend(this._options, options);
	    this._init(element);
	    this._establishConnection();
	    this._renderFrame();
	    this._registerEventListener();
	    // set status to attached
	    this._editElement.data(_mvelo2.default.FRAME_STATUS, _mvelo2.default.FRAME_ATTACHED);
	    // store frame obj in element tag
	    this._editElement.data(_mvelo2.default.FRAME_OBJ, this);
	  }
	  ...
	  _init(element) {
	    ...
	    if (!this._editElement.is('body') && this._editElement.closest('body').data(_mvelo2.default.DYN_IFRAME)) {
	      var html = this._editElement.closest('html');
	      if (!html.data('M-STYLE')) {
	        var style = (0, _jquery2.default)('<link/>', {
	          rel: 'stylesheet',
	          href: _mvelo2.default.extension.getURL('content-scripts/framestyles.css')
	        });
	        // add style
	  ...
	  _renderFrame() {
	    ...
	    (0, _jquery2.default)(window).on('resize', this._setFrameDim.bind(this));
	  ...
	  _closeFrame(finalClose) {
	    this._eFrame.fadeOut(function () {
	      ...
	      if (finalClose === true) {
	        this._port.disconnect();
	        this._editElement.data(_mvelo2.default.FRAME_STATUS, null);
	      } else {
	        this._editElement.data(_mvelo2.default.FRAME_STATUS, _mvelo2.default.FRAME_DETACHED);
	      }
	      this._editElement.data(_mvelo2.default.FRAME_OBJ, null);
	    }.bind(this));
	    return false;
	  }
	  ...
	  _showMailEditor() {
	    this.emit('eframe-display-editor', {
	      text: this._getEmailText(this._editorType == _mvelo2.default.PLAIN_TEXT ? 'text' : 'html')
	    });
	  }

	  _getRecipients() {
	    return this._currentProvider.getRecipients(this._editElement).then(recipients => this.emit('eframe-recipients', { recipients }));
	  }

	  _establishConnection() {
	    this._port = _mvelo2.default.extension.connect({ name: 'eFrame-' + this.id });
	  }

	  _html2text(html) {
	    html = (0, _jquery2.default)('<div/>').html(html);
	    // replace anchors
	    html = html.find('a').replaceWith(function () {
	      return (0, _jquery2.default)(this).text() + ' (' + (0, _jquery2.default)(this).attr('href') + ')';
	    }).end().html();
	    ...
	    html = html.replace(/<(.+?)>/g, ''); // remove tags
	    html = html.replace(/\n{3,}/g, '\n\n'); // compress new line
	    return (0, _jquery2.default)('<div/>').html(html).text(); // decode
	  }
	  ...
	  _setMessage(msg, type) {
	    if (this._emailTextElement.is('textarea')) {
	      ...
	      msg = _mvelo2.default.util.decodeHTML(msg);
